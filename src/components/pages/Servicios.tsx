...
const serviciosData = [
  ...
  },
  {
    ...
    icon: <PrecisionManufacturingIcon sx={{ fontSize: 35, color: '#0b3b7a' }} />,
...
function Servicios() {
  return (
    <MainLayout>
      ...
      <Box sx={{ bgcolor: '#3F3E3A', py: 10, color: '#fff', textAlign: 'center' }}>
        <Container maxWidth="md">
          <Typography variant="overline" sx={{ letterSpacing: 3, fontWeight: 700, color: '#D4D0C4' }}>
            Nuestras Soluciones
          </Typography>
          <Typography variant="h2" sx={{ fontWeight: 900, mb: 3, fontSize: { xs: '2.5rem', md: '3.5rem' } }}>
            Soluciones Integrales en Transporte y Logística
          </Typography>
          <Divider sx={{ width: 80, height: 4, bgcolor: '#fff', mx: 'auto', mb: 4 }} />
        </Container>
      </Box>
        ...
        <Container maxWidth="lg">
          <Grid container spacing={6}>
            {serviciosData.map((s, index) => (
              <Grid key={s.title} size={{ xs: 12 }}> {/* Usamos el ancho completo de la línea para el efecto horizontal */}
                <Card 
                  elevation={0}
                  sx={{ 
                    display: 'flex',
                    flexDirection: { xs: 'column', md: index % 2 === 0 ? 'row' : 'row-reverse' }, // Alterna imagen izquierda/derecha
                ...
                >
                  ...
                  <CardContent sx={{ 
                    ...
                    <Typography 
                      variant="h4" 
                      sx={{ fontWeight: 900, color: '#3F3E3A', mb: 2, textTransform: 'uppercase' }}
                    >
                      {s.title}
                    </Typography>
          ...
          >
            <Box>
              <Typography variant="h4" sx={{ fontWeight: 900, color: '#3F3E3A', mb: 1 }}>
                ¿Necesitás una cotización a medida?
              </Typography>
              <Typography variant="body1" sx={{ color: '#3F3E3A', opacity: 0.8 }}>
                Contamos con la experiencia necesaria para adaptar nuestra logística a tus requerimientos específicos.
              </Typography>
            </Box>
            <Button
